...
fn render_aggregate_screen<D: Display>(
    ...
    match aggregate.op {
        AggregateOp::Integral => draw_tall_glyph::<D>(fb, 0, &INTEGRAL_TOP, &INTEGRAL_BOTTOM),
        AggregateOp::Sum => draw_tall_glyph::<D>(fb, 0, &SIGMA_TOP, &SIGMA_BOTTOM),
    }
    ...
    let body_cols = D::WIDTH.saturating_sub(body_start);
    let body_chars = body_cols / font::CHAR_ADVANCE;
    render_ascii_pretty::<D>(fb, 0, body_start, tail_for_line(aggregate.body, body_chars));

    if let Some(result) = result {
        render_ascii_pretty::<D>(fb, 1, body_start, b"=");
        render_ascii_pretty::<D>(
            fb,
            1,
        ...
        let dx_col = D::WIDTH.saturating_sub(font::CHAR_ADVANCE * 2);
        render_ascii_pretty::<D>(fb, 1, dx_col, &dx);
    }
}
...
fn draw_page_glyph<D: Display>(
    ...
    let page_count = D::HEIGHT / 8;
    if page >= page_count || col + font::GLYPH_WIDTH > D::WIDTH {
        return;
    }

    let offset = page * D::WIDTH + col;
    for (i, &column) in columns.iter().enumerate() {
        fb.as_mut()[offset + i] = column;
    }
    if col + font::GLYPH_WIDTH < D::WIDTH {
        fb.as_mut()[offset + font::GLYPH_WIDTH] = 0;
    }
}

fn draw_tall_glyph<D: Display>(fb: &mut D::Buffer, col: usize, top: &[u8; 5], bottom: &[u8; 5]) {
    draw_page_glyph::<D>(fb, 0, col, top);
    draw_page_glyph::<D>(fb, 1, col, bottom);
}

fn clear_framebuffer<D: Display>(fb: &mut D::Buffer) {
    for byte in fb.as_mut() {
        *byte = 0;
    }
}
