...
fn put_glyph<D: Display>(fb: &mut D::Buffer, page: usize, col: usize, glyph: &[u8; 5]) {
    if page >= D::HEIGHT / 8 || col + 5 > D::WIDTH {
        return;
    }
    let off = page * D::WIDTH + col;
    for (i, &g) in glyph.iter().enumerate() {
        if off + i < fb.as_mut().len() {
            fb.as_mut()[off + i] = g;
        }
    }
    if col + 5 < D::WIDTH {
        let gap = off + 5;
        if gap < fb.as_mut().len() {
            fb.as_mut()[gap] = 0;
        }
    }
}
...
fn put_char<D: Display>(fb: &mut D::Buffer, page: usize, col: usize, byte: u8) {
    ...
    if col > D::WIDTH || col + font::GLYPH_WIDTH > D::WIDTH {
        return;
    }
...
pub fn build_grid(mat: &Matrix, grid: &mut DisplayGrid) {
    ...
    for r in 0..rows {
        ...
        // Left bracket
        let left = if rows == 1 || r == 0 || r == rows - 1 {
            b'['
        } else {
            b'|'
        };
        ...
        // Right bracket
        if pos < MAX_ROW_LEN {
            let right = if rows == 1 || r == 0 || r == rows - 1 {
                b']'
            } else {
                b'}'
            };
...
pub fn render_matrix<D: Display>(
    ...
    for b in fb.as_mut().iter_mut() {
        *b = 0;
    }

    let row_len = grid.row_len as usize;
    let top = row_off.min(total_rows.saturating_sub(2));
    let bot = if total_rows == 1 {
        top
    } else {
        (top + 1).min(total_rows - 1)
    };

    let max_co = if row_len > VIEWPORT_W {
        row_len - VIEWPORT_W
    } else {
        0
    };
    ...
    for i in 0..VIEWPORT_W {
        ...
        if px + font::GLYPH_WIDTH > D::WIDTH {
            break;
        }
    ...
    if arrow_px + font::GLYPH_WIDTH > D::WIDTH {
        return;
    }
