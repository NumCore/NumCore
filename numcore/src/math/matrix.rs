...
impl Matrix {
    ...
    pub fn minor(&self, r: usize, c: usize) -> Option<Self> {
        ...
        Some(Self { data, rows: (n - 1) as u8, cols: (n - 1) as u8, kind: MatrixKind::Mat })
    }
    ...
    pub fn cofactor(&self) -> Option<Self> {
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                let sign = if (i + j) % 2 == 0 { fp::SCALE } else { -fp::SCALE };
                data[i * n + j] = fp::multiply(det, sign)?;
            }
        }
        Some(Self { data, rows: self.rows, cols: self.cols, kind: MatrixKind::Mat })
    }
