...
pub fn ln_gamma(z: i64) -> Option<i64> {
    ...
    if frac_part == fp::FIXED_HALF {
        ...
        let ln_n_fact  = if n == 0 { 0 } else { ln_factorial(fp::from_integer(n as i64))? };
...
pub fn binomial_probability(n: i64, k: i64, p: i64) -> Option<i64> {
    ...
    if k > n                            { return None; }
    if p <= 0 || p >= fp::FIXED_ONE    { return None; }
    if n & (fp::SCALE - 1) != 0        { return None; }
    if k & (fp::SCALE - 1) != 0        { return None; }
...
fn incomplete_gamma_series(a: i64, x: i64) -> Option<i64> {
    ...
    for n in 1..MAX_GAMMA_ITER {
        ...
        if term.abs() < series.abs() / 1_000_000_000 { break; }
    }
...
pub fn chi_squared_cdf(x: i64, k: i64) -> Option<i64> {
    ...
    incomplete_gamma_series(a, half_x)
        .map(|p| p.clamp(0, fp::FIXED_ONE))
}